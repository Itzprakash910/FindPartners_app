import React from 'react';
import { Profile } from '../types';
import MostViewedProfileCard from './MostViewedProfileCard';
import Carousel from './Carousel';

const EyeIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.522 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.022 7-9.542 7-4.478 0-8.268-2.943-9.542-7z" />
    </svg>
);

interface MostViewedProfilesProps {
    profiles: Profile[];
    onViewProfile: (profile: Profile) => void;
}

const MostViewedProfiles: React.FC<MostViewedProfilesProps> = ({ profiles, onViewProfile }) => {
    if (profiles.length === 0) {
        return null;
    }

    return (
        <div className="mb-12">
            <h3 className="text-2xl font-bold text-rose-800 dark:text-rose-400 mb-4 flex items-center">
                <EyeIcon />
                Trending Profiles
            </h3>
            <Carousel>
                {profiles.map(profile => (
                    <MostViewedProfileCard
                        key={profile.id}
                        profile={profile}
                        onViewProfile={onViewProfile}
                    />
                ))}
            </Carousel>
        </div>
    );
};

export default MostViewedProfiles;